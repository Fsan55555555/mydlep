...
const AIPlayer_1 = require("../Misc/AIPlayer");
...
class HaxxorClassic extends AbstractBoss_1.default {
    constructor(game) {
        ...
        this.orbitRate = (Math.random() < .5 ? -1 : 1) * HaxxorClassic.BASE_ORBIT;
        this.rotationRate = (Math.random() < .5 ? -1 : 1) * HaxxorClassic.BASE_ROTATION;
        this.shapeVelocity = HaxxorClassic.BASE_VELOCITY;
        this.timer = 60;
        this.isTurning = 0;
        ...
        this.styleData.values.color = 13;
        ...
        for (let i = 0; i < 8; ++i) {
            ...
            base.tick = (tick) => {
                ...
                if (base.ai.state === 0)
                    base.positionData.values.angle = angle + this.positionData.values.angle;
            };
        }
    }
    ...
    moveAroundMap() {
        ...
        this.positionData.values.angle += this.rotationRate;
    ...
    tick(tick) {
        super.tick(tick);
        this.styleData.values.zIndex = 1;
        this.moveAroundMap();
        // sizeFactorへの代入を削除し、計算値をローカル変数として使用
        const sizeFactor = this.physicsData.values.size / 50;
        if (this.ai.state === 1) {
            this.timer--;
            if (this.timer === 0) {
                if (this.healthData.values.health <= this.healthData.values.maxHealth / 5) {
                    const tonk = new AIPlayer_1.default(this.game, this);
                    ...
                    tonk.relationsData.values = this.rootParent.relationsData.values;
                    tonk.styleData.values.color = this.rootParent.styleData.values.color;
                    tonk.super = true;
                    this.timer = 60;
                }
                else {
