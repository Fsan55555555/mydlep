...
class Titan extends AbstractBoss_1.default {
    constructor(game) {
        ...
        this.orbitRate = (Math.random() < .5 ? -1 : 1) * Titan.BASE_ORBIT;
        this.rotationRate = (Math.random() < .5 ? -1 : 1) * Titan.BASE_ROTATION;
        this.shapeVelocity = Titan.BASE_VELOCITY;
        this.timer = 60;
        this.isTurning = 0;
    ...
    tick(tick) {
        ...
        this.moveAroundMap();
        // sizeFactorへの代入を削除し、必要な場合は計算値として使用
                ...
                if (this.ai.target instanceof TankBody_1.default) {
                    ...
                    if (this.healthData.values.health <= this.healthData.values.maxHealth / 5) {
                        if (this.timer2 >= 150) {
                            ...
                            this.posX = this.positionData.values.x;
                            this.posY = this.positionData.values.y;
                            this.state = 3;
                            this.timer2 = 0;
                        }
                    }
